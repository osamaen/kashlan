...
<!----- header div end ------->
	...
	<div class="span12">
		...
		foreach ($files as $file) {
			...
			//add in thumb folder if not exist
			...
			<?php }else{ ?>
				    ...
				    <figcaption>
						...
						class="erase-button"
						<?php if($delete_folder){ ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo  $cur_dir .$file; ?>','<?php echo $thumbs_path.$subdir .$file; ?>'); $(this).parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>">
			...
			if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
				...
				//add in thumb folder if not exist
				$thumb_path=dirname( __FILE__ ).DIRECTORY_SEPARATOR."thumb".DIRECTORY_SEPARATOR.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
...
?>
				...
				<figure>
					...
					<figcaption>
					    ...
					    <?php if($delete_file){ ?>onclick=" if(confirm('<?php echo lang_Confirm_del; ?>')){ delete_file('<?php echo  $cur_dir .$file; ?>','<?php echo $thumbs_path.$subdir .$file; ?>'); $(this).parent().parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>"><i class="icon-trash <?php if(!$delete_file) echo 'icon-white'; ?>"></i></a>
					    </form>
					</figcaption>
				</figure>
